...
interface CampoMultiplasEscolhasProps {
  pergunta: PerguntaServicoResposta;
  opcoesSelecionadasIds: string[];
  onChange: (opcoesIds: string[]) => void;
  erro?: string;
}

export function CampoMultiplasEscolhas({
  pergunta,
  opcoesSelecionadasIds = [],
  onChange,
  erro,
}: CampoMultiplasEscolhasProps) {
  const toggleOpcao = (id: string) => {
    if (opcoesSelecionadasIds.includes(id)) {
      onChange(opcoesSelecionadasIds.filter((item) => item !== id));
    } else {
      onChange([...opcoesSelecionadasIds, id]);
    }
  };
    ...
    <div className="space-y-2">
      ...
      <div className="space-y-2">
        {pergunta.opcoes.map((opcao) => {
          const selecionada = opcoesSelecionadasIds.includes(opcao.id);
          return (
            <button
              key={opcao.id}
              type="button"
